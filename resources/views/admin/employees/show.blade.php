                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <img src="{{ asset('storage/assets/users/' . $employee->image) }}" alt="Profile Picture"
                                ...
                                <ul class="list-group">
                                    ...
                                    @foreach ($employee->tasks as $task)
                                        @if ($task->status == 'to do')
                                            ...
                                            <li class="list-group-item list-group-item-danger">{{ $task->name }} - {{ $task->status }}</li>
                                        @elseif ($task->status == 'in progress')
                                            @php $inProgressCount++; @endphp
                                            <li class="list-group-item list-group-item-warning">{{ $task->name }} - {{ $task->status }}</li>
                                        @elseif ($task->status == 'done')
                                            @php $doneCount++; @endphp
                                            <li class="list-group-item list-group-item-success">{{ $task->name }} - {{ $task->status }}</li>
                                        @endif
                                    @endforeach
                                </ul>
                                <p><strong> Total tasks:</strong>  {{ $employee->tasks->count() }}</p>
                                <p><strong>To do:</strong> <span class="badge bg-danger">{{ $toDoCount }}</span></p>
                                <p><strong>In progress: </strong><span class="badge bg-warning text-dark">{{ $inProgressCount }}</span></p>
